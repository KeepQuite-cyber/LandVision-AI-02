...
class AITools:
    ...
    def execute(intent):
        ...
        if action == "owner_details":
            return AITools.owner_details(intent.get("plot_number"))

        if action == "village_summary":
            return AITools.village_summary(intent.get("village"))

        if action == "filter_land_use":
            return AITools.filter_land_use(intent.get("land_use"))
    ...
    def search_plot(plot_number):
        plots = PlotService.get_by_plot_number( plot_number)
        if not plots.exists():
            return { "reply": f"Plot {plot_number} was not found."}
        plot = plots.first()
        return {
            "reply" : f"Plot {plot.plot_number} found successfully.",
            "action" : "highlight_plot",
    ...
    def owner_details(plot_number):
        ...
        if not plots.exists():
            return {
                "reply": f"Plot {plot_number} does not exist."
            }
    ...
    def village_summary(village_name):
        plots = PlotService.search(village_name)
        if not plots.exists():
            return {
                "reply": f"No village named {village_name} was found."
            }
        village = plots.first().village
        village_plots = PlotService.get_by_village(village.id)
        return {
            "reply":
                (
                    f"{village.name} has "
    ...
    def filter_land_use(land_use):
        plots = PlotService.get_by_land_use( land_use)
        if not plots.exists():
            return {
                "reply": f"No {land_use} plots found."
            }
        plot_numbers = list(
            plots.values_list("plot_number", flat=True)
        )
        return {
            "reply":
                (
